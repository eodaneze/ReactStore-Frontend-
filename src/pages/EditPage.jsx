...
const EditPage = () => {
   ...
   const updateProduct = async(e) => {
       ...
       setIsLoading(true)
       try{
          const response = await axios.put(`http://localhost:3000/api/${id}`, product);
  ...
  return (
    ...
    <form onSubmit={updateProduct}>
        <div className='space-y-2'>
            <div>
               <label>Name</label>
               <input type="text" value={product.name} onChange={(e) => setProduct({...product, name:e.target.value})}  className='w-full block border p-3 text-gray-600 rounded focus:outline-none focus:shadow-outline focus:border-blue-200 placeholder:gray-400' placeholder='Enter Product name' />
            </div>
            <div>
